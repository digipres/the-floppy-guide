...
8-inch floppy disks were the earliest and widely adopted removable magnetic storage medium and were particularly common in minicomputers and mainframes from the 1970s and early 1980s. These disks typically store between 80 KB and 1.2 MB, depending on the density and encoding. Drives from [<u>Shugart</u> <u>Associates</u>](https://forum.vcfed.org/index.php?threads/selecting-an-8-floppy-drive.56007/), especially the SA800 and SA801 series, remain highly regarded due to their robust construction and broad compatibility. When sourcing 8‑inch drives today, it’s advisable to select units that avoid exposing internal mains voltages (some earlier models used line power internally for spindle motors and logic boards). Drives with external, low-voltage power supplies are not only safer to work on but also far easier to adapt to floppy controllers, this is normally 24V. A good overview of 8-inch drives and disks can be found in [<u>this video</u>](https://www.youtube.com/watch?v=oL0LXSE1jeM). Work on how to set up and run an 8-inch drive in an archival setting was done by Denise de Vries, which includes [<u>a conference paper</u>](https://phaidra.univie.ac.at/detail/o:931070) and [<u>blog series</u>](https://openpreservation.org/blogs/an-8-floppy-disk-challenge/).

### **5.25‑inch floppy disks** 

These drives dominated the home and office PC market throughout the late 1970s into the early 1990s. The earliest 5.25‑inch drives (and disks) were single-sided, Single Density, supporting only 35 or 40 tracks and capacities around 160–180 KB. Later double-sided, Double Density (DSDD) drives supported 320 or 360 KB per disk, and High Density (HD) drives pushed this to 1.2 MB. For the best versatility today, target an 80‑track High Density (1.2 MB) drive. These can reliably read both 40‑track DSDD and 80‑track HD media because their heads are narrower, allowing them to track within the wider grooves of 40‑track disks. TEAC (notably the FD-55 series) and EPSON drives are frequently praised for durability and precision, making them good models to keep an eye out for.

Be aware of certain exceptions: some Double Density (DD) disks; especially those written in non-PC systems, rely on differing spindle speeds or track geometries and can only be reliably imaged with a native DD drive. This means that not every DD disk will read on a HD drive. Some drives are capable of dual speeds, configured through jumpers to allow for 300 rpm (DD) and 360 rpm (HD).
...
However, as has already become clear when buying drives, there are a number of disks that the Greaseweazle struggles with to capture and these are namely the Apple formatted disks. If you have these disks in your collection, you may need to use an [<u>Applesauce</u>](https://applesaucefdc.com/) controller. As stated before, it is still possible to capture Apple disks using a Greaseweazle and a PC/Shugart 5.25-inch or 3.5-inch drive, but there will certainly be exceptions. It should also be noted that the Greaseweazle floppy controller is compatible with the [<u>Fluxengine</u> <u>software</u>](https://cowlark.com/fluxengine/index.html), this will be discussed in more detail in the Imaging section of this guide.

For completeness, a list of other floppy controllers is available below, some of these floppy controllers are no longer live projects and will only be available secondhand. It should be noted that depending on your location and collection that one of the floppy controllers below is more suited to your imaging process.
...
However, one must know what type of disk is being imaged to get a proper image, other controllers automatically detect this. The software can also be difficult to install on certain systems, such as Ubuntu, due to dependencies not clear in the instructions and unclear error messages. When buying this controller it is possible to buy cables alongside it, which makes it easier to ensure you have the right cables for the right drive.
...
If you want to reuse connectors from other machines, verify whether your controller hardware requires a particular cable. Some OEM ribbon cables might have a twist in them. This was done for certain systems that needed a way to designate where each disk drive was in the order of things. If your controller requires a straight-through cable, using a twisted cable will not work..
...
**5.25-inch Drive** → 34-pin cable with edge-card connection + Molex and PSU → Floppy Controller – If you can, acquire the correct drives for the density of your disk. Otherwise go for the most modern HD drive.

**3-inch Drive** → 26-pin cable to 34-pin cable (female on both sides) + custom power (flipped 4-pin connecter) to PSU → Floppy Controller.

**3.5-inch Drive** → 34-pin cable + 4-pin to 4-pin power cable connected to floppy controller → Floppy Controller – If you can, acquire the correct drives for the density of your disk. Otherwise go for the most modern HD drive. You can also consider using a USB floppy drive, but this is at your own risk and is not recommended for preservation purposes.

**Apple Disk II** → 20-pin ribbon (Disk II drive), both power and data → Applesauce – If not possible to acquire Apple II drive, you could try normal 5.25-inch drive with a floppy controller.

**Apple Superdrive 3.5 drive** → DB-19 cable for both power supply and data (this is permanently attached to the drive) → Applesauce - If not possible to acquire Superdrive drive, you could try a regular 3.5-inch drive.
...
As an important note, some of the following cleaning equipment would be considered hazardous, including isopropyl alcohol. Especially when working in a memory institution it is worth checking with facilities management to ensure you are using these chemicals in a well-ventilated and safe environment. Some institutions may have a Conservation department where these types of chemicals are already being used and policies are already in place, it is worth having a conversation with these departments before proceeding with this work.

## 3.1 Cleaning Floppy Disk Drives

Even if a drive appears functional, regular cleaning is essential to prevent read/write errors and protect your disks from contamination. If you have acquired a new drive, even if you have heard from the source that it is functional, it is well worth giving it a clean before use, especially the heads.

Start by cleaning the exterior, this will prevent any future dust or debris on the casing getting trapped in the internal parts of the drive. Disconnect the drive from power and use a soft, lint-free cloth lightly dampened with 70–90% isopropyl alcohol to clean the casing. Avoid introducing moisture through vents or openings. A gentle puff of compressed air can remove loose dust from vents and crevices, but use it sparingly to avoid forcing debris deeper inside.
...
For disks with only light surface contamination, such as dust or smudges, gentle cleaning is usually sufficient. The most commonly used and widely accepted method involves applying high-purity isopropyl alcohol (70-80%) to a lint-free cloth or swab, then lightly wiping the magnetic surface in a straight, radial motion from the hub toward the outer edge. This technique avoids scratching the magnetic layer while lifting away dust or residue. For very light dust, a cloth lightly moistened with [<u>distilled water and a drop of mild soap</u>](https://blog.worldofjani.com/?p=3009) may be used in a similar fashion, followed immediately by drying with a clean cloth. In either case, care must be taken to prevent liquid from entering the interior of the disk jacket, and the disk must be allowed to dry completely before use.

In more serious cases, particularly where mold is visible or foreign material appears to be inside the jacket, it may be necessary to carefully open the disk casing to access the magnetic media directly. This should only be attempted in a clean, dry environment and with an understanding of the risks involved. Once the disk is open, the magnetic medium can be gently cleaned on both sides using a clean swab dampened with isopropyl alcohol. Where the disk has been exposed to water or long-term humidity, rinsing the magnetic media with distilled water followed by air drying may also be appropriate. Once fully dried, the cleaned medium can be reinserted into a donor jacket, preferably taken from a known-good disk, and then tested or imaged immediately. These steps, though more invasive, can sometimes recover data from disks that would otherwise be unreadable.

Other sources to also have a look at in regards to cleaning are David Tenenholtz’s [<u>lightning talk</u>](https://osf.io/zrx2e/), [<u>this forum about cyclomethicone</u>](https://forum.vcfed.org/index.php?threads/heres-a-mystery-for-you-floppy-disk-destroying-floppy-drives.75647/), [<u>this lighting talk at iPRES 2024</u>](https://www.digipres.org/publications/ipres/ipres-2024/papers/the-floppy-disks-in-the-garage-physical-preservation-conditions/) and [<u>this blog</u>](https://nlwmablab.wordpress.com/2014/07/30/104/) by Emma Towner.

## 3.4 Maintaining Floppy Disk Drives
...
Finally, attention should be given to storage and handling. Drives should be kept in a clean, dust-free environment between uses. Some users choose to house their drives in enclosures or casings, to prevent accidental damage, though enclosed drives can be more difficult to clean and service. These can be available online, for example [<u>this case</u>](https://www.ebay.com.au/itm/326005089529?_skw=greaseweazle+drive+enclosure&itmmeta=01K3SCDQFTDAV2HCMT4ACVBHWS&hash=item4be76ae4f9:g:kGcAAOSws6RlyPNW&itmprp=enc%3AAQAKAAAA4FkggFvd1GGDu0w3yXCmi1dHigGaoc%2FvaWWvZmAXTvo4Dg3MI6QVHehMx7OWymMUQxYoOybw2GlrIGkTmowW2nE7qwkzFn61PBWrH6cKijD8k3p0o%2FULc4AfCiPojyap2wyHj64KnhTAlt7hOkXYEAKH0kam6ZliEaeH1lkrm64ORscuwK%2FoMihIlwoPUzdw%2BiH421fymuoHr7z8KJxQnDugtCpPTPGG00DND%2FTxpePAI264De7kYgoWnYaifjZwaEx6qrdNGj18W4OPIB6xCQy0JcomB%2B0JdtQNhxejttYp%7Ctkp%3ABk9SR4j4tqyeZg) for a Greaseweazle and drive which also includes a power supply. When storing a drive for an extended period, or during transport, it is advisable to insert a blank floppy disk into the drive slot. This helps protect the read/write heads from accidental contact or dust accumulation.
...
Flux stream imaging with Greaseweazle is performed by connecting a compatible floppy drive to the device and executing a read command from the command line. The Greaseweazle software can output flux formats:
...
As a recommendation, it is best to consult whatever tool you are going to use in order to process the diskette to see what inputs it accepts. If you are not sure, then you should choose either HFE (which can then be converted into many other types of streams using the HxC software) or choose to

create RAW flux streams. In the case of RAW, the tracks are split into folders, making it easier to swap out tracks and parts if better images are made at a future point. To stitch these various ‘reads’ together it is as simple as copying and pasting folders from one read to another to get a complete picture, this is currently not possible for the SCP format.

Another format to consider is [<u>HxC Floppy Emulator (HFE)</u>](https://hxc2001.com/floppy_drive_emulator/HFE-file-format.html) , which was created as a universal floppy disk image format and is a partially processed format. It was created with the use of emulators in mind but holds a lot of metadata. [<u>It is also significantly smaller in size</u>](https://github.com/keirf/greaseweazle/wiki/Supported-Image-Types) than an SCP and RAW format because HFE stores an interpreted, emulator-friendly representation of tracks and sectors, discarding the detailed flux-timing information preserved in RAW flux-level captures, which retain all magnetic transitions. If you have more experience working with floppy disks, this may be a format to consider.
...
If simultaneous capture is not supported by the floppy controller, it is still preferable to record a complete flux stream. Capturing only raw flux ensures that all magnetic transitions are preserved, including those associated with copy-protection schemes or other unusual disk behaviours. However, without format checking, the quality of the capture cannot be immediately verified, and random errors may also be recorded.

There are drawbacks to flux reading. Many flux readers do not attempt any error determination or correction. They offer the ability to do multiple passes on a track, and save each version of that track, but that can lead to exorbitantly large images which just contain the same damaged read. With this, one might try to look for individual tools written for a specific system, as they would be more attuned to the specific disk type being read and may offer error correction. In the case of disks for the TRS-80, for example, there is a utility called GW2DMK which has the ability to read the diskette and determine, on each read, if all of the sectors on each track have a valid CRC and, if not, to retry those sectors, even going so far as to cache the good sectors and if it can, on subsequent passes, successfully read any previously bad sectors, it would add those to the list, and ultimately output the best possible track, even if it never had a successful track read on any pass.
...
<img src="./media/image1.jpg" style="width:6.26772in;height:3.29167in" />
...
Testing a disk image is also a helpful step, this can be done by mounting the image in an emulator and check whether the software loads or operates as expected. Different emulators will be available for different systems and types of disk images. [<u>The EaaSI project</u>](https://eaasi.gitlab.io/program_docs/eaasi-operations-handbook/charter/what-is-eaasi.html) provides an overview of emulators that can be used, this is especially helpful if the disk image seems to contain an executable.
